...
def color_jitter(batch_points, brightness=0.02, contrast=0.02, saturation=0.02):
    """
    点云颜色扰动增强
    Args:
        batch_points: (B, N, 6) 输入点云批次
        brightness: 亮度扰动强度
        contrast: 对比度扰动强度
        saturation: 饱和度扰动强度
    Returns:
    ...
    rgb = batch_points[..., 3:].float() / 255.0  # 归一化到[0,1]
    
    # 亮度扰动
    if brightness > 0:
        brightness_factor = torch.rand(
            (batch_points.size(0), 1, 1), 
            device=device
    ...
    # 恢复原始范围并保持其他维度不变
    jittered_points = batch_points.clone()
    jittered_points[..., 3:] = rgb.clamp(0, 1) * 255
    
    return jittered_points


def random_point_dropout(batch_points, max_dropout_ratio=0.1):
    """
    随机点丢弃+加噪增强
    Args:
        batch_points: (B, N, 6) 输入点云批次
    ...
    Returns:
        (B, N, 6) 丢弃部分点后的点云（保持原始点数，用零填充）
    """
    device = batch_points.device
    B, N, _ = batch_points.shape
    
    # 为每个样本生成不同的丢弃比例
    dropout_ratios = torch.rand(B, device=device) * max_dropout_ratio
    keep_ratios = 1 - dropout_ratios
    
    # 生成随机掩码
    keep_nums = (N * keep_ratios).long()
    ...
    sorted_indices = torch.argsort(random_values, dim=1)
    mask = torch.zeros(B, N, dtype=torch.bool, device=device)
    for i in range(B):
        mask[i, sorted_indices[i, :keep_nums[i]]] = True
    
    # 应用掩码并用零填充丢弃的点
    dropped_points = torch.zeros_like(batch_points)
    dropped_points[mask] = batch_points[mask]
    
    return dropped_points

def random_point_aug(batch_points, max_dropout_ratio=0.1, noise_std=0.002, noise_clip=0.006):
    """
    随机点丢弃+加噪增强
    Args:
        batch_points: (B, N, 6) 输入点云批次
        max_dropout_ratio: 最大丢弃比例
        noise_std: 噪声的标准差
        noise_clip: 噪声的最大绝对值限制
    Returns:
        (B, N, 6) 增强后的点云（保持原始点数，丢弃点用零填充）
    """
    device = batch_points.device
    B, N, _ = batch_points.shape
    ...
    # 随机噪点增强
    noisy_points = batch_points.clone()
    noise = torch.randn_like(noisy_points[..., :3]) * noise_std
    ...
    # 应用掩码并用零填充丢弃的点
    augmented_points = torch.zeros_like(noisy_points)
    augmented_points[mask] = noisy_points[mask]
...
def pc_normalizer(xyz):
    
    min_vals, _ = torch.min(xyz, axis=1, keepdims=True)  # (B, 1, 3)
    max_vals, _ = torch.max(xyz, axis=1, keepdims=True)  # (B, 1, 3)
    ...
    max_ranges, _ = torch.max(ranges, axis=-1, keepdims=True)  # (B, 1, 1)
    
    max_ranges = torch.where(max_ranges == 0, 
                           torch.ones_like(max_ranges),
                           max_ranges)
    
    normalized_xyz = (xyz - center) / (max_ranges / 2.0)
    
    return normalized_xyz

def normalize_pc_points(points, add_rgb):
    """归一化点云坐标"""
    if add_rgb:
        xyz = points[..., :3]
        rgb = points[..., 3:]
        normalized_xyz = pc_normalizer(xyz)
        normalized_rgb = 2*(rgb / 255. - 0.5)
        points[..., :3] = normalized_xyz
        points[..., 3:] = normalized_rgb
    else:
        points = pc_normalizer(points)

    return points

def pc_normalizer_fix(xyz, max_val, min_val):
    
    # 确保max_val和min_val是torch张量并具有正确的形状
    if not isinstance(max_val, torch.Tensor):
        max_val = torch.tensor(max_val, device=xyz.device, dtype=xyz.dtype)
    if not isinstance(min_val, torch.Tensor):
        min_val = torch.tensor(min_val, device=xyz.device, dtype=xyz.dtype)
    
    # 重塑为(B, 1, 3)以便广播
    max_val = max_val.view(1, 1, 3)
    min_val = min_val.view(1, 1, 3)
    
    # 计算中心点和范围
    center = (max_val + min_val) / 2  # (1, 1, 3)
    ranges = max_val - min_val        # (1, 1, 3)
    
    # 找到每个点的最大范围(跨xyz三个维度)
    max_ranges, _ = torch.max(ranges, dim=-1, keepdim=True)  # (1, 1, 1)
...
def normalize_pc_points_fix(points, add_rgb, max_val, min_val):
    """归一化点云坐标"""
    if add_rgb:
        xyz = points[..., :3]
        rgb = points[..., 3:]
        normalized_xyz = pc_normalizer_fix(xyz, max_val, min_val)
        normalized_rgb = 2*(rgb / 255. - 0.5)
        points[..., :3] = normalized_xyz
        points[..., 3:] = normalized_rgb
    else:
        points = pc_normalizer_fix(points, max_val, min_val)

    return points

def pc_normalizer_centroid(xyz, max_val, min_val):
    ...
    # 归一化处理
    normalized_xyz = (xyz - centroid) / (max_ranges / 2)
    
    return normalized_xyz

def normalize_pc_points_centroid(points, add_rgb, max_val, min_val):
    """归一化点云坐标"""
    if add_rgb:
        xyz = points[..., :3]
        rgb = points[..., 3:]
        normalized_xyz = pc_normalizer_centroid(xyz, max_val, min_val)
        normalized_rgb = 2*(rgb / 255. - 0.5)
        points[..., :3] = normalized_xyz
        points[..., 3:] = normalized_rgb
    else:
        points = pc_normalizer_centroid(points, max_val, min_val)
