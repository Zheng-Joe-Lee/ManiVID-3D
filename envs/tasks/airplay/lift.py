...
_CONFIG_FILE_NAME = 'airplay/lift.json'

def airplay_lift(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
...
class Lift(BaseTask):
    ...
    def get_reward(self, physics):
        ...
        # grasp reward
        grasped = physics.check_grasp('object_block')
        if grasped:
            reward += 0.25
        # lift reward
        if physics.check_lift('object_site', margin=0.73):
            reward = 2.25
        # reward scale
        reward /= 2.25
